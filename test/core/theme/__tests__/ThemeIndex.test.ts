...
import * as themeModule from '../../../../src/core/theme';
...
describe('Theme Index', () => {
  ...
  describe('API Consistency', () => {
    ...
    test('should not export implementation details', () => {
      ...
      // Check that no internal modules are exported (exact matches, not substrings)
      const internalModules = ['internal', 'providers', 'factories'];
      internalModules.forEach(module => {
        exports.forEach(exportName => {
          expect(exportName.toLowerCase()).not.toBe(module);
        });
      });

      // Check for specific implementation patterns
      const implementationPatterns = ['Impl', 'Internal', 'Private'];
      implementationPatterns.forEach(pattern => {
        exports.forEach(exportName => {
          expect(exportName).not.toContain(pattern);
        });
      });
    });
  });
