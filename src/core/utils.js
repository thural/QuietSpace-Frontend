 ...
 * @property {Object} [services] - Services configuration
 */
 ...
 * 
 * @param {CoreSystemStatus} status - Core system status
 * @returns {string} Health status string
 */
export function checkCoreSystemHealth(status) {
 ...
 */
export function getCoreSystemMetrics(status) {
    const serviceCount = Object.keys(status.services).length;
    const healthyServices = Object.values(status.services).filter(Boolean).length;
 ...
 */
export function formatCoreSystemStatus(status) {
 ...
 * 
 * @param {Partial<CoreConfig>} [overrides] - Configuration overrides
 * @returns {CoreConfig} Default core configuration
 */
export function createDefaultCoreConfig(overrides) {
    const defaultConfig = {
        cache: {
            maxSize: 1000,
            defaultTtl: 3600000,
    ...
    if (overrides) {
        if (overrides.cache) {
            mergedConfig.cache = { ...defaultConfig.cache, ...overrides.cache };
        }
