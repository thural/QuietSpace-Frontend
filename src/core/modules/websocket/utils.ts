 ...
 */
export function validateWebSocketConfig(config: unknown): string[] {
    const errors: string[] = [];
    ...
    // Validate URL
    if (!(config as Record<string, unknown>).url || typeof (config as Record<string, unknown>).url !== 'string') {
        errors.push('WebSocket URL is required and must be a string');
        return errors;
    }

    const url = (config as Record<string, unknown>).url as string;
    ...
    if ((config as Record<string, unknown>).reconnectInterval !== undefined) {
        const reconnectInterval = (config as Record<string, unknown>).reconnectInterval;
    ...
    if ((config as Record<string, unknown>).maxReconnectAttempts !== undefined) {
        const maxReconnectAttempts = (config as Record<string, unknown>).maxReconnectAttempts;
        if (typeof maxReconnectAttempts !== 'number' || maxReconnectAttempts < 1) {
  ...
  Timestamp: ${new Date().toISOString()}`;
    ...
    export function createWebSocketError(
        ...
        const error = new Error(message) as Error & { code?: string; details?: unknown; timestamp?: number };
        error.code = code;
