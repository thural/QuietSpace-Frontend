...
import { ConfigurationWatcher, ConfigurationHotReloadManager, ConfigurationChangeEvent } from '../config/ConfigurationWatcher.js';
import { IAuthConfig } from '../interfaces/authInterfaces.js';
 ...
 * @property {Function} get - Get configuration value
 * @property {Function} getAll - Get all configuration values
 * @property {Function} validate - Validate configuration
 * @property {Function} watch - Watch configuration changes
 * @property {Function} unwatch - Unwatch configuration changes
 * @property {Function} [set] - Set configuration value
 * @property {Function} [reset] - Reset configuration
 * @property {Function} [getDefaults] - Get default values
 */

/**
 * Configuration change event interface
 * @typedef {Object} ConfigurationChangeEvent
 * @property {string} type - Event type (added, modified, deleted)
...
describe('ConfigurationWatcher', () => {
    ...
    describe('File Pattern Matching', () => {
        test('should include files matching patterns', () => {
            const shouldInclude = watcher.shouldIncludeFile('/config/auth.json');
            expect(shouldInclude).toBe(true);
        });

        test('should exclude files matching ignore patterns', () => {
            const shouldInclude = watcher.shouldIncludeFile('/node_modules/package.json');
            expect(shouldInclude).toBe(false);
        });

        test('should handle complex patterns', () => {
            const shouldInclude = watcher.shouldIncludeFile('/config/auth.development.json');
            expect(shouldInclude).toBe(true);
        });
    });
});
...
describe('ConfigurationHotReloadManager', () => {
    ...
    describe('Configuration Change Handling', () => {
        test('should handle JSON configuration changes', (done) => {
            ...
            manager.handleJsonConfigChange(event).then(() => {
        ...
        test('should detect configuration type from file path', () => {
            const detectType = manager.detectConfigType;
    ...
    describe('Integration', () => {
        test('should handle full configuration reload', async () => {
            ...
            await manager.handleGenericConfigChange(event);
