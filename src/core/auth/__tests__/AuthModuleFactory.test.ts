...
jest.mock('../repositories/LocalAuthRepository');
jest.mock('../loggers/ConsoleAuthLogger');
jest.mock('../metrics/InMemoryAuthMetrics');
jest.mock('../security/EnterpriseSecurityService');
jest.mock('../providers/JwtAuthProvider');
jest.mock('../providers/OAuthProvider');
jest.mock('../plugins/AnalyticsPlugin');
jest.mock('../plugins/SecurityPlugin');
...
describe('AuthModuleFactory - Environment Configuration', () => {
    ...
    describe('createForEnvironment', () => {
        ...
        it('should fallback to default when environment config fails', async () => {
            ...
            const originalLoadAuthConfiguration = require('../config/AuthConfigLoader').loadAuthConfiguration;
            require('../config/AuthConfigLoader').loadAuthConfiguration = jest.fn().mockRejectedValue(new Error('Config not found'));
            ...
            require('../config/AuthConfigLoader').loadAuthConfiguration = originalLoadAuthConfiguration;
        });
    });
