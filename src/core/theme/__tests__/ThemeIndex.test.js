...
jest.mock('../composer.js', () => ({
  ThemeComposer: jest.fn(),
  themeComposer: jest.fn(),
}));

jest.mock('../variants.js', () => ({
  getTheme: jest.fn(),
  getThemeVariants: jest.fn(),
  defaultTheme: { colors: { primary: '#000' } },
}));

jest.mock('../enhancers/themeEnhancers.js', () => ({
  enhanceTheme: jest.fn(),
}));

jest.mock('../styledUtils.js', () => ({
  Container: jest.fn(),
...
describe('Theme Index', () => {
  ...
  describe('Black Box Pattern Compliance', () => {
    ...
    test('should export interfaces as types only', () => {
      ...
      expect(typeof (themeModule).EnhancedTheme).toBe('undefined');
      expect(typeof (themeModule).ThemeConfig).toBe('undefined');
    });
  });
});
