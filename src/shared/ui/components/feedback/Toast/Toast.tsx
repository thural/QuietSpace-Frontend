...
import { IToastProps, IToastItem } from './interfaces';
import {
...
export class Toast extends PureComponent<IToastProps, IToastState> {
  ...
  private renderToast = (item: IToastItem) => {
    ...
    return (
      <div
        key={item.key}
        css={toastItemStyles(theme, item.type, size)}
      ...
      >
        <div css={toastContentStyles(theme)}>
          {showIcons && (
            <div css={toastIconStyles(theme)}>
          ...
          <div>
            {item.title && (
              <div css={toastTitleStyles(theme)}>{item.title}</div>
            )}
            <div css={toastMessageStyles(theme)}>{item.message}</div>
          </div>

          {item.closable && (
            <button
              css={toastCloseButtonStyles(theme)}
        ...
        {item.showProgress && (
          <div css={toastProgressStyles(theme)}>
            <div
              css={toastProgressFillStyles(theme, item.duration || this.props.defaultDuration || 5000)}
            />
          </div>
        )}
      </div>
  ...
  override render() {
    ...
    return (
      <div
        ...
        css={toastContainerStyles(theme, position)}
