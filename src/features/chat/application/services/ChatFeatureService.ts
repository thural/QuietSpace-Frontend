...
export class ChatFeatureService {
  ...
  async createChatWithValidation(chatData: CreateChatRequest, token: JwtToken): Promise<ChatResponse> {
    ...
    return await this.chatDataService.createChat(sanitizedData, token);
  }
  ...
  async sendMessageWithValidation(
    ...
    const result = await this.chatDataService.sendMessage(chatId, sanitizedData, token);
  ...
  async getChatWithMetadata(chatId: ResId, token: JwtToken): Promise<ChatResponse & { metadata: any }> {
    const chat = await this.chatDataService.getChatDetails(chatId, token);
    const participants = await this.chatDataService.getChatParticipants(chatId, token);
    const typingIndicators = await this.chatDataService.getTypingIndicators(chatId);
  ...
  async getUserChatSummary(userId: string, token: JwtToken): Promise<{
    ...
    const chats = await this.chatDataService.getChats(userId, token);
  ...
  async searchChatsWithFilters(
    ...
    // Basic search
    let results = await this.chatDataService.searchChats(query, userId, token);
  ...
  async handleTypingIndicator(chatId: string, userId: string, isTyping: boolean): Promise<void> {
    await this.chatDataService.setTypingIndicator(chatId, userId, isTyping);
